<div class="container">
    <?php if(!empty($data['history'])): ?>
        ...
        <form action="<?= BASEURL ?>user/history" class="form-inline" method="get">
        ...
        <form class="form-inline mt-2" action="<?= BASEURL ?>user/history" method="get" autocomplete="off">
            <input class="form-control mr-sm-2" type="search" placeholder="Search" name="search" aria-label="Search" id="search">
...
function print() {
    $("<iframe>")
        .hide()
        .attr("src", "<?= BASEURL ?>laporan/pengaduan")
        .appendTo("body");
}
</script>
<script>
<?php 
if(isset($_SESSION['successinput'])){
    echo "alert('Aduan berhasil ditambahkan!')";
    unset($_SESSION['successinput']);
}
?>
</script>